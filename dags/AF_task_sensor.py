...
def transform_data_emoji(raw_file, csv_file):
    ...
    for col in df.columns:
        # Проверка на 'object' (самый частый случай)
        if df[col].dtype == 'object' or pd.api.types.is_string_dtype(df[col]):
            df[col] = (
                df[col]
                .astype(str)
                # убираем все символы, кроме букв, цифр, пробелов и пунктуации
...
with DAG(
    ...
    check_report_task = HttpSensor(
        ...
        endpoint="",
        response_check=response_operator,  # Проверка что отчет готов
        method='GET',
        poke_interval=2,
        timeout=60,
        mode='poke',
    ...
    with TaskGroup("transform_tasks", dag=dag) as transform_group:
        ...
        task_emoji = PythonOperator(
            ...
            outlets=[mongo_ready_data]  # This triggers the update
        )
